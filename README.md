...
- `current` - Hardened the production feedback loop with control-harness roadmap/readiness readback
  ...
  checks, GUI convention tables with icon/text affordance review, hardware-backed Vulkan readiness,
  donor-first web/upstream research for missing routes, stricter enabled-code-map drift/schema
  closeout, generated repo-local map wrappers, donor/provenance closeout, and a GPL-safe Blender
  curves groom-brush study donor.
