...
- Code-map sidecars are bounded maintenance lanes, not parallel source workers. Use one when a drift
  ...
  `agent-tmux codex-code-map-sidecar <repo> <anchor> [focus]`; the drift checker should print that
  command as a concrete hint for uncovered drift and no-map-touch semantic review output. The sidecar
