...
12. For GPU performance work, use the generated optimization loop when available. Start from
    `docs/GPU_OPTIMIZATION_LOOP.md`, create a target table, record a fixed baseline, test one focused
    hypothesis at a time, keep only correct improvements, revert rejected attempts, use `next` for
    move-on decisions, and generate a final report before claiming speedups.
...
- `scripts/run_gpu_optimization_loop.py`: initialize, baseline, attempt, keep/revert, orchestrate, and report evidence-gated GPU optimization sessions.
