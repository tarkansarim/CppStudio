...
# Claude-lane skill sync. Mirrors sync_to_codex.sh but targets ~/.claude/skills and keeps a SEPARATE
# audit log / staging / backup root, per the provider-lane-separation doctrine. Do not merge this with
# the Codex lane. The single CppStudio source under skills/ is Codex-flavored; the Codex lane installs
# it verbatim, while THIS lane applies the Claude provider transform (apply_claude_transform.py /
# claude_install_transform.json) on the way in, so ~/.claude/skills gets a Claude-provider-specific
# copy (host-provider Codex skill-home paths flipped to ~/.claude; supervised-worker/Codex-tool refs
# kept). One source of truth, provider-specific deployment.
...
# Claude has no system skill validator; use the repo-local validator.
...
Sync ${SOURCE_DIR} to the Claude user-level skill copy at ${TARGET_DIR}.

Environment:
  SYNC_CLAUDE_HOME  Defaults to ${HOME}/.claude
  TARGET_DIR        Override the exact installed skill directory
  VALIDATOR         Override the skill validator (defaults to the repo quick_validate_skill.py)
  CPPSTUDIO_CLAUDE_AUDIT_LOG
                    Optional JSONL audit log path. Defaults to
                    ${CLAUDE_HOME_DIR}/cppstudio-claude-install-audit.jsonl for non-dry-run sync.
  ALLOW_SYNC_TARGET_OVERRIDE=1
                    Allow TARGET_DIR outside ${EXPECTED_TARGET_DIR}. Refuses dangerous paths even
                    with this override.

Note:
  Separate Claude lane (per provider-lane-separation doctrine). Does not touch ~/.codex.
EOF
}
...
claude_home_resolved="$(resolve_path "${CLAUDE_HOME_DIR}")"
claude_skills_resolved="$(resolve_path "${CLAUDE_HOME_DIR}/skills")"
...
case "${target_resolved}" in
    "/"|"${home_resolved}"|"${claude_home_resolved}"|"${claude_skills_resolved}"|"${root_resolved}"|"${source_resolved}")
...
if (( dry_run )); then
    ...
    sync_tmp_root="${CPPSTUDIO_CLAUDE_SYNC_TMP_ROOT:-${CLAUDE_HOME_DIR}/.cppstudio-claude-sync-tmp}"
    sync_backup_root="${CPPSTUDIO_CLAUDE_SYNC_BACKUP_ROOT:-${CLAUDE_HOME_DIR}/.cppstudio-claude-skill-backups}"
    if is_within_path "${sync_tmp_root}" "${target_parent}" || is_within_path "${sync_backup_root}" "${target_parent}"; then
        echo "Refusing sync staging or backup root inside scanned Claude skills root:" >&2
    ...
    # Claude-lane provider transform: rewrite host-provider Codex skill-home paths
    # (${CODEX_HOME:-$HOME/.codex}/skills, ~/.codex/skills, ...) to their ~/.claude equivalents so the
    # installed copy is Claude-provider-specific. Supervised-worker/Codex-tool refs are kept by design
    # (see scripts/claude_install_transform.json). File contents change, so regenerate the package
    # manifest before validating, keeping the staged copy internally self-consistent.
    python3 "${ROOT_DIR}/scripts/apply_claude_transform.py" "${staged_target}"
    python3 "${PACKAGE_VALIDATOR}" "${staged_target}" --write-manifest
    python3 "${VALIDATOR}" "${staged_target}"
    python3 "${PACKAGE_VALIDATOR}" "${staged_target}"
    python3 "${ROOT_DIR}/scripts/apply_claude_transform.py" --check "${staged_target}"
