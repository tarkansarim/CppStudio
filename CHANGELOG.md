...
- Hardened the supervisor adversarial-review cadence from a reminder into a mechanical pre-nudge and
  closeout gate. Supervisors must record the last reviewed slice, current slice count since review,
  and next-review debt in the worker watchlist/status; unknown, stale, or absent cadence state blocks
  the next implementation nudge. Risky shader/runtime, GPU sync, UI interaction, persistence,
  generated-project infrastructure, and visible/rendered-path slices can require immediate
  post-implementation review even before the numeric cadence.
...
- Added an acceptance-artifact cutover gate for long visual, reference-render, calibration,
  ...
  narrow read-only `codex exec` stuck probe, perform substantive current web/upstream research into
  the exact stuck layer, and explicitly choose continue or cutover before more local patching. Local
  source-only analysis or token web search is no longer enough after this stall gate. If another
  focused attempt still leaves the final artifact red, a fresh scoped Codex adversarial review is
  required before more patching. Cutover must preserve any user-named upstream, SDK, shader,
  renderer, file-format, or port target; unrelated peer tools are diagnostic references, not
  replacement targets, and conflicting stale-context or peer-tool lanes must be filtered out before
  user-facing cutover choices are presented.
